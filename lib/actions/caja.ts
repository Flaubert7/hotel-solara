...
export async function editarIngreso(id: number, formData: FormData) {
  ...
  const { error } = await supabase.from('caja_ingresos').update({
    fecha, categoria, nombre, detalle, forma_pago, monto_gs, monto_usd, nro_cbte
  }).eq('id', id)
  if (error) throw new Error(error.message)
...
export async function editarEgreso(id: number, formData: FormData) {
  ...
  const { error } = await supabase.from('caja_egresos').update({
    fecha, nro_recibo, nro_factura, detalle, monto_gs, monto_usd
  }).eq('id', id)
  if (error) throw new Error(error.message)
...
export async function editarRetiro(id: number, formData: FormData) {
  ...
  const { error } = await supabase.from('caja_administracion').update({
    fecha, retiro_gs, retiro_usd, descripcion
  }).eq('id', id)
  if (error) throw new Error(error.message)
